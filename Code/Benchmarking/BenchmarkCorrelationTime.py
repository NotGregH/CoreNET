...
def run_benchmarks():
	...
	for i in filesList:
		...
		subprocess.call(["C:\WINDOWS\system32\WindowsPowerShell\\v1.0\powershell.exe", "python Network_Correlation.py " +
			"-d Benchmark.db -p 0.05 -e .\..\JoanDataV2.txt -o Test.SIF " + 
			"-g .\Test_Lists\\" + i + " | Out-File -Append  Correlation_Time_noFilter.txt"], shell=True) 
		
		subprocess.call(["C:\WINDOWS\system32\WindowsPowerShell\\v1.0\powershell.exe", "python Network_Correlation.py " +
			"-d Benchmark.db -p 0.05 -e .\..\JoanDataV2.txt -o Test.SIF -l .\Edge_lists\ppEdges.txt " + 
			"-g .\Test_Lists\\" + i + " | Out-File -Append  Correlation_Time_Filter.txt"], shell=True) 
